...
setlocale(LC_ALL, 'hr_HR.utf-8');
	...
	{		
		$id = addSlashes($_GET["id"]);
			$sql = ("SELECT*FROM slike_naslova WHERE id_slike = $id ");	
			$rezultat = mysqli_query($veza, $sql);
			if($redak = mysqli_fetch_array($rezultat))	
			{
				$idClanka = $redak["clanak_id"];
			...
			$sql = ("DELETE FROM slike_naslova WHERE id_slike = $id "); 
		...
		header("Location: priprema_slike_naslova.php?id=" . $idClanka );
	}
?>
</body>
</html> 
